...
- Decide `Project Layer`, `User-wide`, or `Undetermined` scope before choosing a capture path or target type.
- If a Project Layer exists and broader applicability is uncertain, prefer `.adaptive-agents/retrospectives/inbox/` and record what evidence would justify escalation.
- Use the canonical `retrospectives/inbox/` only for established user-wide lessons.
...
- If creating a note, use the inbox selected by the scope decision and the filename `YYYY-MM-DD-short-title.md`.
- If creating a note, set `Status: Captured`.
- If creating a note, record scope, scope rationale, observation, evidence, impact, proposed target within that scope, and a dogfood check.
...
1. Created file
2. Selected scope and rationale
3. Captured lesson in one sentence
4. Why it remains `Captured`
5. Suggested dogfood or triage check
